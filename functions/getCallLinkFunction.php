...
function getCallLinkFunction($username, $token)
{
	...
	if ($existsUser == true)
	{
		...
		if ($userAuthenticated == true)
		{
			...
			{
				$resultValue->errormessage = "MYSQL: Could not prepare statement ".$query1;
				$error = true;
			}
			else
			{
			...
			$query2 = 'SELECT callconnecting.idcall FROM callregistration, callconnecting WHERE callregistration.idcall=callconnecting.idcall AND idclient=? AND callregistration.calltimeout>? AND callconnecting.calltimeout>? AND callaccepted<? AND callregistration.callfinished=callregistration.callrequested';
			if (!($stmt = $link->prepare($query2)))
			{
				$resultValue->errormessage = "MYSQL: Could not prepare statement ".$query2;
				$error = true;
			}
			else
			{
