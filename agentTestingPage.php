...
angular.module("myApp", ['ngAnimate'])
  .controller("mainController", function ($scope)
  {
	...
	function callgetLinkService(FilterParameter)
	{
		...
		xmlhttp.open("POST", "getCallLinkServiceAgent.php", true);
	...
	function callAcceptCallService(FilterParameter)
	{
		xmlhttp = new XMLHttpRequest();
		xmlhttp.onreadystatechange = function() 
		{
			...
			if (this.readyState == 4 && this.status == 200) 
			{
				...
				var myObj = JSON.parse(this.responseText);
					composedtxt = myObj.Result;
					if (composedtxt=="Agent accepted a call")
					{
		...
		xmlhttp.open("POST", "acceptCall.php", true);
		var sentstring = FilterParameter;
		xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		xmlhttp.send(sentstring);
	}
	
	function callFinishCallService(FilterParameter)
	{
		xmlhttp = new XMLHttpRequest();
		xmlhttp.onreadystatechange = function() 
		{
		...
		xmlhttp.open("POST", "finishCall.php", true);
		var sentstring = FilterParameter;
		xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		xmlhttp.send(sentstring);
	}
	...
	$scope.onGetLinkClick = function () {
		...
		$scope.lastfilter += "&userid="+$scope.userid;
	...
	$scope.onAcceptCall = function () {
		...
		$scope.lastfilter += "&token="+$scope.token;
		clearParentPanel();
		callAcceptCallService($scope.lastfilter);
	}
	   	
	$scope.onFinishCall = function () {
		$scope.lastfilter = "username="+$scope.username;
		$scope.lastfilter += "&idcall="+$scope.roomid;
		$scope.lastfilter += "&token="+$scope.token;
		clearParentPanel();
		callFinishCallService($scope.lastfilter);
	}
		
	var init = function () {
		$scope.username = "Agent001";
...
<form id="ajaxcall">
	Agentname: <input id="username" type="text" ng-model="username"/><br>
	...
	<button type="button" name="Request call" ng-click="onAcceptCall()">Accept call</button><br>
	<button type="button" name="Cancel call" ng-click="onFinishCall()">Finish call</button><br>
